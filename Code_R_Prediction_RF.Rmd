...
```
# "Mise à jour" des variables en facteurs ordinals
```{r}
...
```{r}
func_F1_complet$moy_F1_complet
```
...
```
```{r}
func_F1_normalite$moy_F1_normalite
```
...
```{r}
func_F1_normcorr$moy_F1_normcorr
```
...
F1_comp_groupe4=function(x,data){
  ...
  F1_comp=foreach(i=1:x,.combine=cbind,.packages=c("caret"))%dopar%{
        ...
        modelcomp3 <- caret::train(class_jar_gout_4~., data = train[,-c(39,40,41)],method="rf", ntree = 500,trControl=train.control)
...
```
# Optimisation des modèles retenus par GINI.
# Modèle 1
```{r}
library(randomForest)
modelnormalite <- randomForest(class_jar_gout_2~., data = sensogroupe2[,-c(30,29,27)], ntree = 500, importance=T)
varImpPlot(modelnormalite,type=2)
```

```{r}
data_normaliteGINI=data.frame(sensogroupe2[,c("HALEINE_SAINE4","HALEINE_SAINE5","FRAIS4","AROME4","FRAIS5","AROME5","SUCRE3","class_jar_gout_2")])
```
...
```
```{r}
func_F1_normalite_GINI$moy_F1_normalite
```

# Modèle 2
```{r}
modelnormcorr2 <-randomForest(class_jar_gout_3~., data = sensogroupe3[,-c(23,21,20)], ntree = 500, importance=T)
varImpPlot(modelnormcorr2,type=2)
```

```{r}
data_normcorrGINI=data.frame(sensogroupe2[,c("HALEINE_SAINE5","AROME4","AROME5","ODEUR1","ASTRINGENT4","VISCOSITE2","ACIDE3","class_jar_gout_3")])
```
...
F1_normcorr_groupe3GINI=function(x,data){
  set.seed(123)
    F1_normcorr=foreach(i=1:x,.combine=cbind,.packages=c("caret"))%dopar%{
        ...
        pred3=predict(modelnormcorr2, test3[,-c(8)])
        F1_class1=as.data.frame(caret::confusionMatrix(pred3,test3$class_jar_gout_3,  mode = "everything")$byClass)$F1[1]
        F1_class2=as.data.frame(caret::confusionMatrix(pred3,test3$class_jar_gout_3,  mode = "everything")$byClass)$F1[2]
        F1_class3=as.data.frame(caret::confusionMatrix(pred3,test3$class_jar_gout_3,  mode = "everything")$byClass)$F1[3]
...
```{r}
func_F1_normcorr_groupe3GINI$moy
```

# Modèle 3

```{r}
modelnormcorr3 <-randomForest(class_jar_gout_4~., data = sensogroupe3[,-c(22,21,20)], ntree = 500, importance=T)
varImpPlot(modelnormcorr3,type=2) 
```

```{r}
data_normcorrGINI2=data.frame(sensogroupe2[,c("HALEINE_SAINE5","AROME4","AROME5","ODEUR1","AMER3","ASTRINGENT4","class_jar_gout_4")])
```
...
```{r}
func_F1_normcorr_groupe4GINI$moy
```
